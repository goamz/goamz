...
func (l *CloudWatchLogs) CreateLogGroup(name string) error {
	_, err := l.doQuery(target("CreateLogGroup"), NewGroupQuery(name))
	return err
}

// CreateLogStream - see http://goo.gl/1IEVYN
func (l *CloudWatchLogs) CreateLogStream(groupName, streamName string) error {
	query := NewGroupQuery(groupName).AddLogStreamName(streamName)
...
func (l *CloudWatchLogs) DeleteLogGroup(name string) error {
	_, err := l.doQuery(target("DeleteLogGroup"), NewGroupQuery(name))
	return err
}
...
func (l *CloudWatchLogs) GetLogEvents(groupName, streamName string,
	...
	*GetLogEventsResult, error) {
	// define query
	query := NewGroupQuery(groupName).AddLogStreamName(streamName)
	query.AddStartFromHead(startFromHead).AddLimit(limit)
	query.AddStartTime(startTime).AddEndTime(endTime)
	// perform query
...
func (l *CloudWatchLogs) PutRetentionPolicy(
	...
	query := NewGroupQuery(groupName).AddRetentionInDays(retentionInDays)
...
func (l *CloudWatchLogs) DeleteRetentionPolicy(name string) error {
	_, err := l.doQuery(target("DeleteRetentionPolicy"), NewGroupQuery(name))
	return err
}
...
func (l *CloudWatchLogs) PutMetricFilter(filterName, filterPattern,
	...
	// define query
	query := NewGroupQuery(groupName).AddMetricTransformations(metrics)
...
func (l *CloudWatchLogs) DeleteMetricFilter(groupName, filterName string) error {
	query := NewGroupQuery(groupName).AddFilterName(filterName)
